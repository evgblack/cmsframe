...
export let DateTimeResolver = new GraphQLScalarType({
  name: 'DateTime',

  description: 'Use JavaScript Date object for date/time fields. ISO-8601',

  serialize(value) {
    // return check(value).toJSON();
    return value;
  },
...
function check(value){
  ...
  if (!(v instanceof Date) && type !== 'string' && type !== 'number') {
    throw new TypeError(
